...
\begin{equation}\tag{1}\label{eq:probtwo}
	p(x_1, x_2) = 
	\begin{cases}
...
#### Write a computer code that samples the PDF of (\ref{eq:probtwo}) using the gibbs sampling algorithm. Plot the PDF and the samples you obtain from the Gibbs algorithm on a 2d contour plot, similarly to Figure 3 in the course note 2. 
...
\begin{equation}\tag{2}\label{eq:partc}
	f(y;X_1, X_2) = \sin{(4 \pi X_1 y)} + \cos{(4 \pi X_2 y)} \quad y \in [0,1]
\end{equation}

where $X_1$ and $X_2$ are random variables with joint PDF given by (\ref{eq:probtwo}).
...
partc();
```
![Plot of 50000 samples on a 500 spatial grid of evenly distributed values of y from 0 to 1](q2partc.png){fig-align="center"
width="100%"}
...
\begin{equation}\tag{3}\label{eq:probthree}
	\bar{X}_N = \frac{X_1 + ... + X_N}{N} \quad \text{sample mean}
\end{equation}
...
![Plot of 1000 samples from the cauchy distribution](q31000.png){fig-align="center"
width="100%"}
![Plot of 100000 samples from the cauchy distribution](q3100000.png){fig-align="center"
width="100%"}
