...
validatepione(π);
```
![Validation of the stieltjes algorthim: Plot of numerical over analytical solution of the first orthogonal polynomial](hw5_files/media/validationpione.png)
...
![Heatmap showing that the matrix of innerproducts with respect to the weight function of all polynomials generated is diagonal.](hw5_files/media/heatmapdiagonal.png)
...
plotpolynomials(π)
```
![Plot of polynomials within the support [-1, 1]](hw5_files/media/plotpolynomials.png)
...
![PDF of $\eta(\xi(\omega))$ by sampling 50,000 times via inverse CDF approach and applying the transformation \eqref{eta}](hw5_files/media/question2a.png)
...
![The PDF ploted using Kernel Density Estimation for values M = {1, 2, 4, 6}](hw5_files/media/question2b.png)
...
![Barplot of the bootstrap mean and variance from MC for $\eta$ and $\eta_6$](hw5_files/media/question2c.png)
...
function question3a()
    ...
    for idx in CartesianIndices(Ep1pjpk)
        (j, k) = idx.I
        Ep1pjpk[j, k] = integ(x -> πn[2](x) * πn[j](x) * πn[k](x) * μ(x), sup)
    end
...
![The mean and variance of \eqref{eight} in $t \in [0, 3]$](hw5_files/media/question3a.png)
...
![PDF for various times found by the method of relative frequencies](hw5_files/media/question3b.png)
...
![The mean and variance of $x(t;\omega)$ using Monte Carlo method](hw5_files/media/validationmeanvar.png)

![The PDF for specific times using Monte Carlo method](hw5_files/media/validationPDF.png)

## Figures


![The PDF of $\xi$](hw5_files/media/PDFone.png){#fig-pdfxi}
